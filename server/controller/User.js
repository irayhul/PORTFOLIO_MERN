...
export const addWeight = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: "Added To Weight-Lose Gallery",
    });
  } catch (error) {
    return res.status(400).json({
      success: false,
...
export const addGallery = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: "Added Photos in Gallery",
    });
  } catch (error) {
    return res.status(400).json({
      success: false,
...
export const deleteWeight = async (req, res) => {
  try {
    ...
    const photo = user.weight.find((photo) => photo._id == id);

    await cloudinary.v2.uploader.destroy(photo.image.public_id);

    user.weight = user.weight.filter((photo) => photo._id != id);

    await user.save();

    res.status(200).json({
      success: true,
      message: "Deleted Weight-Lose Image from Images ",
    });
  } catch (error) {
    return res.status(400).json({
      success: false,
...
export const deleteGallery= async (req, res) => {
  try {
    ...
    const images = user.gallery.find((image) => images._id == id);

    await cloudinary.v2.uploader.destroy(images.image.public_id);

    user.gallery = user.gallery.filter((images) => images._id != id);

    await user.save();

    res.status(200).json({
      success: true,
      message: "Deleted Images from Gallery",
    });
  } catch (error) {
    return res.status(400).json({
      success: false,

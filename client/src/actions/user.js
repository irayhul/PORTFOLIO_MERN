...
export const addWeight = (title,image) => async (dispatch) => {
  try {
    dispatch({
...
export const deleteWeight = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`/api/v1/admin/weight/${id}`);
...
export const addGallery = (title,subtitle, image) => async (dispatch) => {
  try {
    dispatch({
...
export const deleteGallery = (id) => async (dispatch) => {
  try {
    dispatch({
      type: "DELETE_GALLERY_REQUEST",
    });

    const { data } = await axios.delete(`/api/v1/admin/gallery/${id}`);
